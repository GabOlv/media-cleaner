...
(async () => {
  ...
  try {
    ...
    await page.getByText("Uma pequena revisão por vez.", { exact: true }).waitFor();
    ...
    assert.equal(await page.getByRole("radio", { name: "20", exact: true }).count(), 0, "the batch presets should stop at 15");
    assert.equal(await page.getByRole("radio", { name: "Personalizado", exact: true }).count(), 1, "custom batch should remain a single aligned option");
